...
const DiabetesPatientRegister = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (Object.keys(formErrors).length > 0) {
      ...
      toast({
        title: "Validation Error",
        description: "Please fill in all required fields correctly across all steps.",
        variant: "destructive",
      });
        ...
        return (
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6" >
              {[
                { id: "HbA1c_Level", label: "HbA1c Level", unit: "%", icon: Heart, step: "0.1" },
                { id: "Fasting_Blood_Glucose", label: "Fasting Glucose", unit: "mg/dL", icon: Activity, step: "0.1" },
                { id: "Blood_Pressure", label: "Blood Pressure", unit: "mmHg", icon: Heart, step: "1" },
                { id: "Cholesterol", label: "Cholesterol", unit: "mg/dL", icon: Activity, step: "1" },
                { id: "BMI", label: "BMI", unit: "kg/m²", icon: User, step: "0.1" },
                { id: "Albuminuria", label: "Albuminuria", unit: "mg/dL", icon: Activity, step: "0.1" },
              ].map(({ id, label, unit, icon: Icon, step }) => (
                <div key={id} className="space-y-2">
                  ...
                  <Input
                    ...
                    className={cn(
                      "transition-all duration-200 hover:border-green-300 focus:border-green-500",
                      errors[id as keyof FormErrors] && "border-red-500 focus:border-red-500"
                    )}
                    placeholder={`Enter ${label.toLowerCase()}`}
                  />
                  {errors[id as keyof FormErrors] && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className={cn(
                    "transition-all duration-200 hover:border-green-300 focus:border-green-500",
                    errors.Duration_of_Diabetes && "border-red-500 focus:border-red-500"
                  )}
                  placeholder="Years with diabetes"
                />
                {errors.Duration_of_Diabetes && (
        ...
        return (
          <div className="space-y-4">
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
              <h3 className="font-semibold text-blue-900 mb-3">Patient Summary</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                <div><strong>ID:</strong> {formData.patient_id || "N/A"}</div>
                ...
                <div className="sm:col-span-2"><strong>Mobile:</strong> {formData.mobile_number || "N/A"}</div>
              </div>
            </div>
            
            <div className="bg-green-50 p-4 rounded-lg border border-green-200">
              <h3 className="font-semibold text-green-900 mb-3">Clinical Data</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                <div><strong>HbA1c:</strong> {formData.HbA1c_Level ? `${formData.HbA1c_Level}%` : "N/A"}</div>
                <div><strong>Fasting Glucose:</strong> {formData.Fasting_Blood_Glucose ? `${formData.Fasting_Blood_Glucose} mg/dL` : "N/A"}</div>
                <div><strong>Blood Pressure:</strong> {formData.Blood_Pressure ? `${formData.Blood_Pressure} mmHg` : "N/A"}</div>
                <div><strong>Cholesterol:</strong> {formData.Cholesterol ? `${formData.Cholesterol} mg/dL` : "N/A"}</div>
                <div><strong>BMI:</strong> {formData.BMI ? `${formData.BMI} kg/m²` : "N/A"}</div>
                <div><strong>Albuminuria:</strong> {formData.Albuminuria ? `${formData.Albuminuria} mg/dL` : "N/A"}</div>
                <div><strong>Diabetes Duration:</strong> {formData.Duration_of_Diabetes ? `${formData.Duration_of_Diabetes} years` : "N/A"}</div>
                <div><strong>Visual Acuity:</strong> {formData.Visual_Acuity || "N/A"}</div>
              </div>
            </div>

            <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
              <h3 className="font-semibold text-purple-900 mb-3">Administrative Details</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                <div><strong>Hospital:</strong> {formData.Hospital_name || "N/A"}</div>
                <div><strong>Registration Date:</strong> {formData.Date_of_registration ? format(formData.Date_of_registration, "PPP") : "N/A"}</div>
                <div className="sm:col-span-2"><strong>Previous Visits:</strong> {formData.num_visits || "0"}</div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-center p-4 bg-gray-50 rounded-lg mt-6">
              {!registrationComplete ? (
                ...
                <Button
                  ...
                  className="w-full sm:w-auto px-8 py-3 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg shadow-lg"
                >
                  Upload Retinopathy Eye Images
                </Button>
              )}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
      <div className="flex w-full">
        ...
        {/* Desktop Sidebar */}
        <div className="hidden md:block w-80 h-screen bg-white  border-r border-gray-200 overflow-y-auto">
          <StepperContent />
        </div>
          ...
          <div className="max-w-4xl mx-auto">
            <form onSubmit={handleSubmit} className="space-y-6">
              <Card className="animate-fade-in shadow-lg">
                ...
                <CardContent className="p-4 md:p-8">{renderFormStep()}</CardContent>
              </Card>
