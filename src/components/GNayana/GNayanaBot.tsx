...
import FriendlyBotGreeting from "@/components/GNayana/AnimatedBotGreeting"; // Ensure filename matches
...
// --- Configuration ---
const WEBSOCKET_URL =
  "wss://g-nayan-chatbot-latest.onrender.com/ws/Gnayana_chat"; // Use a constant
...
const GNayanaBot: React.FC = () => {
    ...
    if (lastMessage !== null) {
      ...
      try {
        ...
        if (parsedData && typeof parsedData.text === "string") {
          ...
          console.warn(
            "Received message object without 'text' property:",
            parsedData
          );
  ...
  const handleSendMessage = () => {
    ...
    if (!trimmedMessage || readyState !== ReadyState.OPEN) {
      if (readyState !== ReadyState.OPEN) {
        toast({
          title: "Not Connected",
          description: "Cannot send message, not connected to chat.",
          variant: "destructive",
        });
      }
      return;
  ...
  const handleModeToggle = (newMode: Mode) => {
    ...
    if (messages.length > 0) {
      ...
      toast({
        title: `Switched to ${
          newMode === "assistant" ? "Assistant" : "Chat"
        } Mode`,
  ...
  const [isChatOpen, setIsChatOpen] = useState(false); // Track main chat state

  // Show greeting after delay, only if main chat isn't already open
  useEffect(() => {
    ...
    return () => clearTimeout(timer); // Cleanup on unmount or if chat opens
  }, [isChatOpen]); // Dependency array includes isChatOpen

  const handleTriggerOpenChat = () => {
    setShowGreeting(false); // Hide the greeting immediately
    setIsChatOpen(true); // Update state
    console.log("Attempting to open main chat interface...");
    // ----> Add logic here to *actually* open your GNayanaBot <----
    // This might involve:
    // 1. Calling a method on a ref attached to GNayanaBot.
    // 2. Setting a prop on GNayanaBot (like `isOpen={true}`).
    // 3. Updating global state (if using Zustand, Redux, Context).
    // Example (if GNayanaBot takes an `openChat` prop function):
    // openMainChatFunction();
  };

  const handleCloseChat = () => {
    setIsChatOpen(false);
    // Optionally reschedule greeting?
  };
  return (
    <>
      {/* Bot toggle button */}
      ...
      {isOpen && (
        <div
          className={cn(
            ...
            isMinimized
              ? "bottom-6 w-64 h-16"
              : "bottom-6 w-[350px] md:w-[280px] h-[400px] max-h-[calc(100vh-4rem)]"
          )}
        >
          {/* Header */}
                      ...
                      <div className="flex space-x-2 items-center">
                        ...
                        <Button
                          ...
                          disabled={
                            ...
                            isBotTyping ||
                            readyState !== ReadyState.OPEN
                          } // Disable when typing, empty, or not connected
...
// Example Usage in Parent Component (e.g., Layout.tsx)
// import React, { useState, useEffect } from 'react';
// import FriendlyBotGreeting from './AnimatedBotGreeting'; // Adjust path
// // import GNayanaBot from './'; // Your main chat bot component
...
//       {/* Render your main chat bot - Ensure it can be opened via the logic in handleTriggerOpenChat */}
//        {/* Example: Maybe it always renders but opens based on `isChatOpen` state */}
//       <GNayanaBot /> Or pass props: <GNayanaBot isOpen={isChatOpen} onClose={handleCloseChat} />
//     </div>
//   );
// };
